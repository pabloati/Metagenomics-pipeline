...
```{r}
table(colData(tse_filt)[tse_filt$Fecal_sample_ON_days < 45,"Diagnosis"])/nrow(colData(tse_filt)[tse_filt$Fecal_sample_ON_days < 45,"Diagnosis"])
```
...
ggsave("final_plots/ON_baseline_FAMD.png",width = 4, height = 3, dpi = 300)
```
...
as.data.frame(colData(tse_filt)) %>% 
  ...
  scale_color_manual(values = m.color) +
  ylim(c(0,400))
ggsave("final_plots/ON_baseline_richness.png",width = 7,height = 7, dpi = 300)
```
...
as.data.frame(colData(tse_filt)) %>% 
  ...
  scale_color_manual(values = m.color) +
  ylim(2,5.15)
ggsave("final_plots/ON_baseline_shannon.png",width = 7,height = 7, dpi = 300)
```
...
as.data.frame(colData(tse_filt)) %>% 
  ...
  scale_color_manual(values = m.color) +
  ylim(20,100)
ggsave("final_plots/ON_baseline_FPD.png",width = 7,height = 7, dpi = 300)
```
...
ggsave("final_plots/ON_baseline_BC.png",width = 7,height = 7, dpi = 300)
```
```{r}
permanova_dist("bray")
```
...
ggsave("final_plots/ON_baseline_Jac.png",width = 7,height = 7, dpi = 300)
```
```{r}
...
#ggsave("final_plots/ON_baseline_CB.png",width = 7,height = 7, dpi = 300)
```
```{r}
...
ggsave("final_plots/Permanova_BC.png", width = 7,height = 7, dpi = 300)  
```
...
ggsave("final_plots/ON_baseline_AD.png",width = 7,height = 7, dpi = 300)
```
...
ggsave("final_plots/ON_baseline_phyla.png",width = 7,height = 7, dpi = 300)
```
...
ggsave("final_plots/Volcano_plot.png",height = 7,width = 7,dpi = 300)
```
...
ggsave("final_plots/Pvalue_plot.png",height = 7,width = 7,dpi = 300)
```
...
ggsave("aaTrial.png",width=5,height = 10,dpi =300 ) 
```
...
ggsave("final_plots/Classifier_comparisons.png", dpi = 300)
```
...
ggsave("final_plots/Sup/All_classifier_comparisons.png", dpi = 300)
```
...
ggsave("final_plots/Sup/FAMD_quant_var.png",,width = 2, height = 2,dpi = 300)
```
...
ggsave("final_plots/Sup/FAMD_quali_var.png",width = 2, height = 2,dpi = 300)
```
...
ggsave("final_plots/Sup/Reads_per_step.png",width=4,height=3,dpi = 300)
```
...
ggsave("final_plots/Sup/Rarefaction_curve.png",width = 4, height = 3,dpi = 300)
```
...
ggsave("final_plots/Sup/Rarefaction_diff.png",width = 4, height = 3,dpi = 300)
```
```{r} 
n1 <- frich.df %>% head(n=20) %>% mutate(size = "Original") %>% select(richness,size)
n2 <- as.data.frame(colData(tse_bs)) %>% mutate(size = "Rarefied",richness = observed) %>% select(richness,size)
...
ggsave("final_plots/Sup/rareFilt_phylum_relabund.png",width=7,height = 4, dpi = 300)
```
...
ggsave("final_plots/Sup/rareFilt_FPD.png",width=4,height=4)
```
...
ggsave("final_plots/Sup/rareFilt_PCoA.png",width=4,height=4,dpi=300)
```
...
#ggsave("ON_baseline_DE_heatmap.png",width = 30, height = 20, dpi = 300)
```
...
ggsave("../Presentation/Images/RareFilt_aD.png")
```
...
ggsave("../Presentation/Images/RareFilt_bD_unfiltered.png")
...
ggsave("../Presentation/Images/RareFilt_bD_filtered.png")
```
